...
import (
	...
	userApp "github.com/mmanjoura/nomad/backend/user"
)
...
func (s *AuthService) FindAuthByID(ctx context.Context, id int) (*userApp.Auth, error) {
...
func (s *AuthService) FindAuths(ctx context.Context, filter userApp.AuthFilter) ([]*userApp.Auth, int, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, 0, err
	}
...
func (s *AuthService) CreateAuth(ctx context.Context, auth *userApp.Auth) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	if other, err := findAuthBySourceID(ctx, tx, auth.Source, auth.SourceID); err == nil {
		...
		*auth = *other
		return tx.Commit()
	} else if backend.ErrorCode(err) != backend.ENOTFOUND {
	...
	if auth.UserID == 0 && auth.User != nil {
		...
		if user, err := findUserByEmail(ctx, tx, auth.User.Email); err == nil { // user exists
			auth.User = user
		} else if backend.ErrorCode(err) == backend.ENOTFOUND { // user does not exist
...
func findAuthByID(ctx context.Context, tx *Tx, id int) (*userApp.Auth, error) {
	auths, _, err := findAuths(ctx, tx, userApp.AuthFilter{ID: &id})
	if err != nil {
		return nil, err
	} else if len(auths) == 0 {
		return nil, &backend.Error{Code: backend.ENOTFOUND, Message: "Auth not found."}
	}
	return auths[0], nil
}
...
// Returns ENOTFOUND if auth doesn't exist.
func findAuthBySourceID(ctx context.Context, tx *Tx, source, sourceID string) (*userApp.Auth, error) {
	auths, _, err := findAuths(ctx, tx, userApp.AuthFilter{Source: &source, SourceID: &sourceID})
	if err != nil {
		return nil, err
	} else if len(auths) == 0 {
		return nil, &backend.Error{Code: backend.ENOTFOUND, Message: "Auth not found."}
...
func findAuths(ctx context.Context, tx *Tx, filter userApp.AuthFilter) (_ []*userApp.Auth, n int, err error) {
	...
	auths := make([]*userApp.Auth, 0)
	for rows.Next() {
		var auth userApp.Auth
...
func createAuth(ctx context.Context, tx *Tx, auth *userApp.Auth) error {
...
func updateAuth(ctx context.Context, tx *Tx, id int, accessToken, refreshToken string, expiry *time.Time) (*userApp.Auth, error) {
...
func deleteAuth(ctx context.Context, tx *Tx, id int) error {
	...
	if auth, err := findAuthByID(ctx, tx, id); err != nil {
		return err
	} else if auth.UserID != userApp.UserIDFromContext(ctx) {
		return backend.Errorf(backend.EUNAUTHORIZED, "You are not allowed to delete this auth.")
	}
...
// to the auth object.
func attachAuthAssociations(ctx context.Context, tx *Tx, auth *userApp.Auth) (err error) {
