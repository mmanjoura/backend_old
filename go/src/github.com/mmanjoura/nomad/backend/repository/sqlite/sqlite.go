...
var (
	...
	shopCountGauge = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "app_db_shops",
		Help: "The total number of shops",
	})

	shopMembershipCountGauge = promauto.NewGauge(prometheus.GaugeOpts{
		Name: "app_db_shop_memberships",
		Help: "The total number of shop memberships",
	})
)
...
type DB struct {
	...
	EventService backend.EventService
...
func NewDB(dsn string) *DB {
	db := &DB{
		...
		EventService: backend.NopEventService(),
	}
...
func (db *DB) updateStats(ctx context.Context) error {
	...
	if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM shop;`).Scan(&n); err != nil {
		return fmt.Errorf("shop count: %w", err)
	}
	shopCountGauge.Set(float64(n))

	// if err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM shop_membership;`).Scan(&n); err != nil {
	// 	return fmt.Errorf("shop membership count: %w", err)
	// }
	shopMembershipCountGauge.Set(float64(n))
...
// FormatError returns err as a app error, if possible.
...
func FormatError(err error) error {
	...
	// switch err.Error() {
	// case "UNIQUE constraint failed: shop_membership.shop_id, shop_membership.user_id":
	// 	return nomad.Errorf(nomad.ECONFLICT, "shop membership already exists.")
	// default:
	// 	return err
	// }
