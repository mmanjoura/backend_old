...
var _ appUser.UserService = (*UserService)(nil)
...
func (s *UserService) FindUserByID(ctx context.Context, id int) (*appUser.User, error) {
...
func (s *UserService) FindUsers(ctx context.Context, filter appUser.UserFilter) ([]*appUser.User, int, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, 0, err
	}
...
func (s *UserService) CreateUser(ctx context.Context, user *appUser.User) error {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func (s *UserService) UpdateUser(ctx context.Context, id int, upd appUser.UserUpdate) (*appUser.User, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, err
	}
...
// DeleteUser permanently deletes a user and all owned shops.
...
// Returns ENOTFOUND if user does not exist.
func findUserByID(ctx context.Context, tx *Tx, id int) (*appUser.User, error) {
	a, _, err := findUsers(ctx, tx, appUser.UserFilter{ID: &id})
	if err != nil {
		return nil, err
	} else if len(a) == 0 {
		return nil, &backend.Error{Code: backend.ENOTFOUND, Message: "User not found."}
	}
	return a[0], nil
}
...
// Returns ENOTFOUND if user does not exist.
func findUserByEmail(ctx context.Context, tx *Tx, email string) (*appUser.User, error) {
	a, _, err := findUsers(ctx, tx, appUser.UserFilter{Email: &email})
	if err != nil {
		return nil, err
	} else if len(a) == 0 {
		return nil, &backend.Error{Code: backend.ENOTFOUND, Message: "User not found."}
...
func findUsers(ctx context.Context, tx *Tx, filter appUser.UserFilter) (_ []*appUser.User, n int, err error) {
	// Build WHERE clause.
...
func createUser(ctx context.Context, tx *Tx, user *appUser.User) error {
...
func updateUser(ctx context.Context, tx *Tx, id int, upd appUser.UserUpdate) (*appUser.User, error) {
	// Fetch current object state.
	user, err := findUserByID(ctx, tx, id)
	if err != nil {
		return user, err
	} //else if user.ID != appUser.UserIDFromContext(ctx) {
	// 	return nil, nomad.Errorf(nomad.EUNAUTHORIZED, "You are not allowed to update this user.")
	// }
...
func deleteUser(ctx context.Context, tx *Tx, id int) error {
	...
	if user, err := findUserByID(ctx, tx, id); err != nil {
		...
		return backend.Errorf(backend.EUNAUTHORIZED, "You are not allowed to delete this user.")
	}

	// Remove row from database.
	if _, err := tx.ExecContext(ctx, `DELETE FROM user WHERE id = ?`, id); err != nil {
		return FormatError(err)
	}
	return nil
}
...
func attachUserAuths(ctx context.Context, tx *Tx, user *appUser.User) (err error) {
	if user.Auths, _, err = findAuths(ctx, tx, appUser.AuthFilter{UserID: &user.ID}); err != nil {
