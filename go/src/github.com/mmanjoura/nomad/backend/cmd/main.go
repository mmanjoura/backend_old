...
import (
	...
	"github.com/mmanjoura/nomad/backend/api"
	...
	//"github.com/mmanjoura/app/http/html"
	//"github.com/mmanjoura/app/inmem"
	"github.com/mmanjoura/nomad/backend/repository/sqlite"
...
func main() {
	...
	backend.Version = strings.TrimPrefix(version, "")
...
func (m *Main) ParseFlags(ctx context.Context, args []string) error {
	...
	fs := flag.NewFlagSet("appd", flag.ContinueOnError)
	...
	// home directory. This is a nomad task as configuration files are typing
...
func (m *Main) Run(ctx context.Context) (err error) {
	...
	if m.Config.Rollbar.Token != "" {
		...
		rollbar.SetServerRoot("github.com/mmanjoura/nomad/backend")
		backend.ReportError = rollbarReportError
		backend.ReportPanic = rollbarReportPanic
	...
	attributeService := sqlite.NewAttributeService(m.DB)
	categoryService := sqlite.NewCategoryService(m.DB)
	couponService := sqlite.NewCouponService(m.DB)
	productService := sqlite.NewProductService(m.DB)
	orderService := sqlite.NewOrderService(m.DB)
	settingService := sqlite.NewSettingService(m.DB)
	shippingService := sqlite.NewShippingService(m.DB)
	// categoryTypeService := sqlite.NewCategoryTypeService(m.DB)

	// Attach user service to Main for testing.
	m.UserService = userService
	m.AttributeService = attributeService
	m.CategoryService = categoryService
	m.CouponService = couponService
	m.ProductService = productService
	m.OrderService = orderService
	m.SettingService = settingService
	m.ShippingService = shippingService
	// m.CategoryTypeService = categoryTypeService
	...
	// m.HTTPServer.ShopService = ShopService
	// m.HTTPServer.ShopMembershipService = ShopMembershipService
	// m.HTTPServer.EventService = eventService
	m.HTTPServer.UserService = userService
	m.HTTPServer.AttributeService = attributeService
	m.HTTPServer.CategoryService = categoryService
	m.HTTPServer.CouponService = couponService
	m.HTTPServer.ProductService = productService
	m.HTTPServer.OrderService = orderService
	m.HTTPServer.SettingService = settingService
	m.HTTPServer.ShippingService = shippingService
	// m.HTTPServer.CategoryTypeService = categoryTypeService
	...
	if m.HTTPServer.UseTLS() {
		go func() {
			log.Fatal(api.ListenAndServeTLSRedirect(m.Config.HTTP.Domain))
		}()
	}

	// Enable internal debug endpoints.
	go func() { api.ListenAndServeDebug() }()
...
func rollbarReportError(ctx context.Context, err error, args ...interface{}) {
	if backend.ErrorCode(err) != backend.EINTERNAL {
		return
	}

	// Set user information for error, if available.
	if u := appUser.UserFromContext(ctx); u != nil {
